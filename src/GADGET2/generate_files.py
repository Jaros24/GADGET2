...
def generate_files(run_num:int, length=80, ic=800000, pads=21, eps=7, samps=8, poly=2, overwrite=False):
    """
    ...
    (tot_energy, skipped_events, veto_events, good_events, len_list, trace_list, xHit_list, yHit_list, zHit_list, eHit_list, angle_list) = main(h5file=f, pads=pads, ic=ic, max_length=length)
...
def track_len(xset, yset, zset, max_length=80):
    """
...
def main(h5file, pads, ic, max_length):
    """
    ...
    for i in tqdm.trange(1, num_events+1):
        ...
        cloud_x, cloud_y, cloud_z, cloud_e, veto = remove_outliers(cloud_x, cloud_y, cloud_z, cloud_e, pads, eps, samps)
        if veto == True:
            skipped_events += 1
            continue
        """
        ...
        length, veto_on_length, angle = track_len(cloud_x, cloud_y, cloud_z, max_length=max_length)
